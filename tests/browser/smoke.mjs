...
try {
  ...
  const optionalSeed = page.getByText('Start from prior curation', { exact: true });
  await optionalSeed.waitFor();
  if (await page.getByText('Choose curated JSONL', { exact: true }).isVisible()) throw new Error('Optional prior curation control is too prominent by default');
  await optionalSeed.click();
  await page.getByText('Choose curated JSONL', { exact: true }).waitFor();
  await optionalSeed.click();
  await page.locator('.drop-zone input[type=file]').setInputFiles([annotationA, annotationB]);
  ...
  await page.getByRole('dialog', { name: 'Replace the current comparison?' }).waitFor();
  await page.getByText('A validation error will leave it unchanged.', { exact: false }).waitFor();
  await page.getByRole('button', { name: 'Cancel' }).click();
  if (await page.getByRole('button', { name: 'Confirm text' }).isDisabled()) throw new Error('Confirmation warning cannot be opened while differences remain');
  await page.getByRole('button', { name: 'Confirm text' }).click();
  await page.getByRole('dialog', { name: 'Confirm this text anyway?' }).waitFor();
  await page.getByRole('button', { name: 'Cancel' }).click();
  ...
  if (deleteButtonTitle !== 'Delete selected span (Backspace)') throw new Error(`Delete shortcut is not plain Backspace: ${deleteButtonTitle}`);
